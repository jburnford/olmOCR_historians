...
class Predictor(BasePredictor):
    ...
    def _wait_for_server(self, timeout=600):
        ...
        while time.time() - start < timeout:
            ...
            if self.server_process.poll() is not None:
                ...
                raise RuntimeError(f"vLLM server exited with code {self.server_process.returncode}:\n{stdout[-2000:]}")
            time.sleep(3)
        raise RuntimeError("vLLM server failed to start within timeout")

    def _process_page(self, pdf_path: str, page_num: int) -> str:
        """Render a page and send it to the vLLM server."""
    ...
    def predict(
        ...
        # Assemble markdown
        md_parts = [r.strip() if r else f"[Page {i+1}: no output]" for i, r in enumerate(results)]
        markdown = "\n\n".join(md_parts)
